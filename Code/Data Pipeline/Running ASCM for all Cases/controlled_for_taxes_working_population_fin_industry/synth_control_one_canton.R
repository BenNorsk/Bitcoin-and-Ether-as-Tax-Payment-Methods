...
do_and_save_synth_analysis <- function(canton, data, cutoff) {
    # Do the analysis
    data %>% select(days, canton, companies, treatment, working_population, taxes, `share of financial sector`) 
    asyn <- augsynth((companies ~ treatment | taxes + working_population + `share of financial sector`), canton, days, data,
